...
(function () {
  ...
  /* Where our own assets live. Read from the script's own src rather than
     ...
     their copy instead of reaching back to our CDN. */
  var ASSET_BASE = (function () {
    try { return new URL('.', scriptTag.src).href.replace(/\/$/, ''); }
    catch (e) { return '__CK_CDN__'; }
  })();
  ...
  var DEFAULT_API_BASE = '__CK_API__';
  ...
  var config = {
    ...
    showTyping: true,
    showCitations: false,
    /* The "Powered by ConverseKit" line. TRUE by default, and only ever
       turned off by /health — a paid tier removes it, and that is a
       field in a payload, never a second widget build. One artifact on
       the CDN is what keeps the test coverage of this file meaning
       something. A widget older than the field simply keeps showing it,
       which is the safe direction for the default to fail in. */
    branding: true,
  ...
  function buildDOM() {
    ...
    var powered = el('div', { id: 'aicb-powered',
      innerHTML: 'Powered by <a href="__CK_SITE__" target="_blank" rel="noopener">ConverseKit</a>' });
    ...
    return { root, shadow, wrap, panel, bubble, badge, messages, typing, chips, input, sendBtn, botName, closeBtn, avatar, powered };
  }
  ...
  /* Removed rather than hidden. `display:none` would leave the line in
     the accessibility tree of some assistive tech and in the panel's
     text content, which is exactly the thing a tenant paying for it not
     to be there is paying for. */
  function applyBranding(dom) {
    if (config.branding || !dom.powered || !dom.powered.parentNode) return;
    dom.powered.parentNode.removeChild(dom.powered);
  }
  ...
  function fetchConfig(cb) {
    fetch(API_BASE + '/v1/bots/' + botId + '/health')
      ...
      .then(function (d) {
        ...
        /* Strict false, not falsy: an older Worker that does not send
           the field must leave the default alone. */
        if (d.branding === false) config.branding = false;
