...
export async function retrieve(
  ...
  //
  // The entitlement is checked first and reported the same way. A tier
  // without RAG is not an error and not an empty corpus — it is a bot
  // that answers from its business profile and system prompt, which is
  // exactly what `skipped: 'disabled'` already means downstream.
  if (!entitlementsFor(bot).rag) return { chunks: [], skipped: 'disabled' };
