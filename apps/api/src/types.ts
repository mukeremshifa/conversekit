...
import type { RateLimiter } from './entitlements';
...
export interface Env {
  ...
  /** Cloudflare rate-limiting bindings, one namespace per tier.
   *
   *  A limit is static configuration — it cannot be computed at request
   *  time — so the tiers have to exist as separate bindings even though
   *  everything else about a tier is a row in a table. src/entitlements.ts
   *  picks between them; nothing else may read them directly.
   *
   *  Optional, all three: absent means no limiting, which is the
   *  pre-existing behaviour and must never be the reason a deploy or a
   *  `wrangler dev` session fails. */
  RL_FREE?: RateLimiter;
  RL_PRO?: RateLimiter;
  RL_SCALE?: RateLimiter;
...
export interface Bot {
  id: string;
  org_id: string;
  /** Embedded by selectBot as `organizations(plan)` — the plan, on the
   *  row the caller already has, so src/entitlements.ts stays a pure
   *  function instead of a database round trip. Undefined on rows that
   *  came back from an insert or an update, which do not ask. */
  organizations?: Pick<Organization, 'plan'> | null;
  name: string;
