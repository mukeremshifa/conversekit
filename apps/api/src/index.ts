...
app.get('/v1/bots/:id/health', async (c) => {
  ...
  return c.json({
    ...
    // Tier differences ride here, in a field, and never in a second
    // widget build: one artifact on the CDN, one set of bytes to test.
 ...
 *
 * Not monetisation, even though the allowance now varies by plan —
 * every bot shares one Gemini key and one Workers AI allocation, so an
 * unthrottled endpoint lets anyone holding a bot UUID drain the
 * platform's free tier from a script. Keyed per bot per client so one
 * noisy visitor cannot starve another tenant.
 *
...
async function rateLimited(
  ...
  // Which namespace, and therefore which allowance, is the one thing
  // about a tier that has to be decided at deploy time — see the note
  // at the top of src/entitlements.ts.
  const limiter = chatLimiterFor(c.env, entitlementsFor(bot));
  if (!limiter) return false;

  const client = c.req.header('cf-connecting-ip') ?? c.req.header('x-forwarded-for') ?? 'unknown';
  try {
    const { success } = await limiter.limit({ key: `${bot.id}:${client}` });
    return !success;
  } catch (err) {
...
app.post('/v1/chat', async (c) => {
  ...
  if (await rateLimited(c, pre.bot)) {
  ...
  try {
    ...
    return c.json({
      ...
      orgs:   memberships.map((m) => ({
        ...
        role: m.role,
        // What the plan actually allows, resolved server-side. The
        // dashboard must never re-derive this from `plan` — two copies
        // of the mapping is how a screen ends up offering something the
        // API then refuses.
        entitlements: getEntitlements(m.organizations),
      })),
    });
  } catch (err) { console.error(err); return c.json({ error: 'Database error' }, 502); }
});
...
app.put('/v1/admin/bots/:id', async (c) => {
  ...
  // Vendor entitlement. "A tier locked to one vendor" is a plan
  // question, not a catalog question, so it is asked here rather than
  // in providers/catalog.ts: the catalog describes what exists, the
  // seam decides who may reach it. Refused at write time rather than at
  // resolve time so a tenant learns on the settings screen instead of
  // when their first visitor asks something.
  if ('provider_config' in payload || 'embedding_config' in payload) {
    const current = await getBotForAdmin(c.get('db'), c.req.param('id')).catch(() => null);
    if (!current) return c.json({ error: 'Bot not found' }, 404);
    const ent = entitlementsFor(current);
    for (const field of ['provider_config', 'embedding_config'] as const) {
      const cfg = payload[field] as { vendor?: unknown } | null | undefined;
      const vendor = cfg && typeof cfg === 'object' ? cfg.vendor : undefined;
      if (typeof vendor === 'string' && !vendorAllowed(ent, vendor)) {
        return c.json({ error: `The ${ent.plan} plan cannot use '${vendor}'.`, field }, 403);
      }
    }
  }
...
app.post('/v1/admin/bots/:id/logo', async (c) => {
  ...
  try {
    ...
    if (/widget_config/.test(message)) {
      return c.json({ error: 'Logos need the bots schema (supabase/002_bots.sql), which has not been applied yet.' }, 501);
    }
...
app.post('/v1/admin/bots/:id/documents/upload', async (c) => {
  ...
  try {
    const cap = entitlementsFor(bot).storageBytes;
    const used = await orgStorageBytes(c.get('db'), bot.org_id);
    if (used + file.size > cap) {
      return c.json({
        error: `Storage limit reached — ${mb(used)} of ${mb(cap)} used. Delete a source to free space.`,
        usedBytes: used,
        capBytes:  cap,
      }, 413);
    }
  ...
  try {
    ...
    if (/r2_key|mime_type|size_bytes|documents_source_check/.test(message)) {
      return c.json({ error: 'File uploads need the knowledge schema (supabase/004_knowledge.sql), which has not been applied yet.' }, 501);
    }
    return c.json({ error: 'Could not create document' }, 403);
  }
...
app.post('/v1/admin/bots/:id/profile/backfill', async (c) => {
  ...
  try {
    ...
    if (/profile/.test(message)) {
      ...
      return c.json({ error: 'The business profile needs the bots schema (supabase/002_bots.sql), which has not been applied yet.' }, 501);
    }
    console.error(err);
...
app.get('/v1/admin/bots/:id/retrieval', async (c) => {
  ...
  try {
    ...
    if (/retrieval_log/.test(message)) {
      ...
      return c.json({ error: 'The retrieval report needs the retrieval schema (supabase/005_retrieval.sql), which has not been applied yet.' }, 501);
    }
    console.error(err);
    return c.json({ error: 'Database error' }, 502);
  }
...
app.get('/v1/admin/bots/:id/usage', async (c) => {
  ...
  try {
    ...
    if (/usage_log/.test(message)) {
      ...
      return c.json({ error: 'The usage report needs the usage schema (supabase/006_usage.sql), which has not been applied yet.' }, 501);
    }
    console.error(err);
    return c.json({ error: 'Database error' }, 502);
  }
