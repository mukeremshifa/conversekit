...
async function selectBot(db: AnyDb, botId: string): Promise<Bot | null> {
  // `organizations(plan)` is embedded rather than fetched separately so
  // src/entitlements.ts can stay synchronous: every "may this tenant …"
  // question is then answered off a row the caller already holds. The
  // embed rides the bots → organizations foreign key, and RLS lets a
  // member read their own org, so it adds no round trip on either the
  // service path or the user path.
  const rows = await pgFetch<Bot[]>(db,
    `/bots?select=*,organizations(plan)&id=eq.${encodeURIComponent(botId)}&limit=1`
  );
  return rows[0] ?? null;
}
