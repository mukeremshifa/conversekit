...
// The TTFs are not vendored — assets/fonts/ holds woff2 subsets, which resvg
// cannot read, and fontsource ships no static cuts. So the type stages only
// run for whoever has the three files sitting in the repo root; the icons are
// pure geometry and always run. Rendering type without them does not fail, it
// silently falls back and quietly rewrites the lockups and the OG image in
// the wrong face, so this is a skip and not a warning.
const HAVE_FONTS = FONTS.every((f) => fs.existsSync(f));
 ...
 *  disappears, so the frame thickens and the dot grows — an optical size,
 *  not a scaled-down copy.
 *
 *  The tile radius is then forced by the frame it wraps. The frame's stroke
 *  ends 4 - 4.4/2 = 1.8 from the tile edge, with an outer corner radius of
 *  6.5 + 4.4/2 = 8.7, so only rx = 8.7 + 1.8 holds that 1.8 gap on the
 *  diagonals too. Anything tighter (this was rx 5) cuts the tile corner
 *  inside the frame's arc and the gap roughly doubles there — at which point
 *  the ink stops reading as an outline and starts reading as four black
 *  wedges pinned to the corners, which is what you see in a browser tab. */
const markTileSmall = ({ tile, fg, uid = 's' }) =>
  `<rect width="32" height="32" rx="10.5" fill="${tile}"/>` +
...
if (!HAVE_FONTS) {
  console.log(`Skipped logo.png, logo-dark.png, og.png — missing ${FONTS.filter((f) => !fs.existsSync(f)).map((f) => path.basename(f)).join(', ')} in the repo root.`);
}
...
if (HAVE_FONTS) fs.writeFileSync(path.join(OUT, 'og.png'), rasterize(og, 1200));
